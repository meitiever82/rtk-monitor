...
- [ ] solution status（规则 4/6 的输入）：确认 demo5 rtkrcv 输出 $SAT 的配置方式
      ...
      **注意**：接通 stat 流后天空图仍不会自动点亮——还需扩展 ws-contract §1（`sol` 增加
      逐卫星字段）并修改 `_diagnosis_tick`（main.py）与 `replay.py`；当前占位文案属预期
...
### 前端最终审查台架核对项（final review bench checks）

- [ ] 事件行有级别色条（左边框）且鼠标悬停为手型（cursor: pointer）
- [ ] 同一事件点击两次不出现重复行（事件去重生效）
- [ ] 回到实时后地图无回放轨迹到当前位置的直线（trails/series 已清空重绘）
- [ ] 实时中拖拽缩放时间线，并确认行为符合预期（缩放状态在下一次数据刷新时保留）
- [ ] 状态条各项（徽章/回放标签/卫星/龄期/σ/速度/时钟）间距正常，不粘连
